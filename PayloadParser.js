 ...
 // Verificamos si es un array y si tiene al menos un elemento
  if (Array.isArray(parsedArray) && parsedArray.length > 0) {
    ...
    if (parsed.sensor) {
      ...
      device.endpoints.byAddress("16").updateGenericSensorStatus(parseFloat(parsed.sensor[15]));
      device.endpoints.byAddress("17").updateGenericSensorStatus(parseFloat(parsed.sensor[16])); 
    } else {
